...
//		int[] digits = plusOne(array);
	...
	public static int[] plusOne(int[] digits) {
		for (int i = digits.length - 1; i >= 0; i--) {
			if (digits[i] < 9) {
	//will increase last number if it is less than 9
				digits[i]++;
				return digits;
			}
...
//if only one digit 9 is there, the increase the size of array to 2
//By default the new array is [0,0]
		digits[0] = 1;
//assign to 0th position to 1 and new array is [1,0];
